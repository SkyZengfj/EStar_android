...
public class NewsHeartDetailEAPActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.news_heart_detail_eap);
	...
	private void init() {
		...
		this.runOnUiThread(new Runnable() {
			...
			@Override
			public void run() {
				((TextView)findViewById(R.id.news_heart_detail_eap_title)).setText(hhl.title);
				((TextView)findViewById(R.id.news_heart_detail_eap_content)).setText(hhl.content);
				
				DisplayMetrics metrics = new DisplayMetrics();
				getWindowManager().getDefaultDisplay().getMetrics(metrics);
				int width = (metrics.widthPixels * 660) / 720;
				int height = (metrics.widthPixels * 444) / 720;
				Bitmap bmpbig = asyncLoader.loadBitmap((ImageView)findViewById(R.id.news_heart_detail_eap_img),
						hhl.image_url, width, height, new ImageCallBack() {
				...
				if (bmpbig != null) {
					((ImageView)findViewById(R.id.news_heart_detail_eap_img)).setImageBitmap(bmpbig);
				}
			}
		});
	}
